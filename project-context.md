...
- **Queue source of truth:** `_bmad-output/audit/testsprite-queue.yaml` — `schema_version: 1` ([@contract-v1] Rule 20 surface; 19-7 reads this shape). Each run rotates consumed entries from `queue:` to `history:` (capped at 200, FIFO prune); empty `queue:` triggers refill from `history:` oldest-first. Quota constants (`monthly_budget`, `consumption_cap_pct`, `reserved_credits`) live in the YAML — owner edits the file to switch plan tier, no workflow code change.
...
- **Budget ceiling (AC #3, dual):** `consumption_cap_pct: 80` (120/150 credits per run — guards runaway cases) **AND** `reserved_credits: 30` (floor — guards monthly-cron from starving the manual ad-hoc lane). Workflow stops on whichever hits first; live `testsprite_check_account_info` queried before each case, never trusts the per-entry `credits_estimated: 5` (informational only).
...
- **Commit-message convention** (AC #4 — `git log --grep='chore(testsprite): monthly run'` is the audit-trail filter):
...
- **Failure semantics (AC #6):** workflow exit ≠ 0 only for "human must intervene" — API auth failure, target URL unreachable, or `git push` rejected after 3-retry rebase. Single-case test `fail` is data (recorded in `history.last_status: fail`, exit 0) so Actions email isn't flooded by regressions. For regression alerting, see 19-7 (`testsprite-quota-warning.yml`, ready-for-dev) or a future per-case-fail Issue-opener workflow.
