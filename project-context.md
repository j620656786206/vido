...
AI_TIMEOUT, AI_QUOTA_EXCEEDED, AI_UNAUTHORIZED, AI_MODEL_NOT_FOUND, AI_RETRIES_EXHAUSTED, AI_INVALID_RESPONSE, AI_PROVIDER_ERROR, AI_NOT_CONFIGURED, AI_BUDGET_EXCEEDED
DB_NOT_FOUND, DB_QUERY_FAILED
VALIDATION_REQUIRED_FIELD, VALIDATION_INVALID_FORMAT
SUBTITLE_NOT_FOUND, SUBTITLE_DOWNLOAD_FAILED, SUBTITLE_CONVERT_FAILED, SUBTITLE_EXTRACT_FAILED, SUBTITLE_NO_TEXT_SOURCE, SUBTITLE_TRANSLATE_FAILED, SUBTITLE_TIMESTAMP_MISMATCH, SUBTITLE_TARGET_NOT_WRITABLE, SUBTITLE_EXTRACT_WAIT_ABORTED
...
REQUEST_DUPLICATE, REQUEST_ALREADY_IN_LIBRARY, REQUEST_INVALID_SELECTION
...
TRANSCRIPTION_DISABLED, TRANSCRIPTION_IN_PROGRESS, TRANSCRIPTION_BATCH_RUNNING, TRANSCRIPTION_BATCH_START_FAILED, TRANSCRIPTION_ANALYSIS_RUNNING
NFO_LOCALIZE_DISABLED, NFO_LOCALIZE_FAILED, NFO_REPLACE_NOT_CONFIRMED
```

**Authoritative prefix set (17 sources):** `TMDB_`, `AI_`, `DB_`, `VALIDATION_`, `SUBTITLE_`, `PLUGIN_`, `SCANNER_`, `SSE_`, `LIBRARY_`, `QBITTORRENT_`, `METADATA_`, `DOUBAN_`, `WIKIPEDIA_`, `REQUEST_`, `DVR_`, `TRANSCRIPTION_`, `NFO_`. When adding a new subsystem with its own error codes, extend this list AND sync `_bmad/bmm/workflows/4-implementation/code-review/instructions.xml` Step 3 "Rule 7 Wire Format Check" (both the HTML comment sync date and the inline prefix list).
