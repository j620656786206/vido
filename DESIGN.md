...
38 個母版裡**只有一個是行動專用**（`Component/MobileTabItem`）。20 幾張手機稿各自手畫同一個 Sheet 外殼，沒有母版。這是 `disc-2026-09-missing-component-masters` 排在前面的項目之一。
...
階梯有 16 階：2／4／6／8／10／12／14／16／20／24／28／32／40／48／64／80。**半階（6／10／14／20／28）不是「將就」，是有明確用途的正式階**，見 §Layout 那份對照表（例：20px 是 CJK 按鈕的水平內距，因為中文字身框直接貼到內距邊，需要約 0.25em 的補償）。挑符合用途的那一階，就這樣。
