...
**會花錢的動作要有記號（The Cost-Bearing Action Rule）** 一個會產生實際費用的動作，必須在控制項上帶一個固定記號（`$` 或硬幣圖示）＋預估金額，**而不是靠顏色**。詳情面板的字幕區有三顆一模一樣的次要按鈕——搜尋字幕（免費）、AI 校正（計費）、轉錄英文音軌（計費）——視覺上完全看不出差別，而「花費上限＋同意流程」是 PRODUCT.md 寫的護城河第一層。⚠️ 記號本身尚未實作，追蹤於 `disc-2026-09-no-cost-bearing-component`。
