...
2. **驗版面**：`Get((n,c)=>c.problems && ...)` 數全檔裁切警告。**基準值是 71**（2026-09-11 補上 12 張手機設定分頁與三方 review 修正後，從 59 上升——那 12 個警告全是分頁列刻意的橫向捲動裁切；在那之前是 2026-09-10 逐一修正 Flow A–M 內容從 116 降到 59。剩下的多半是刻意的橫向捲動出血與長頁面的折線下內容），超過就是新的破版，要修到回來為止。
