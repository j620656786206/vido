...
候選清單（F15／F18）把同一條規則套用在**列**上，多出兩種沒有金額的列（dsr-6e-1，2026-09-20）：**這一列的估價缺席**——後端沒有給 `estimated_usd`，那一列選不到、右側寫「無法估價」（就是「停用＋原因」，只是對象從按鈕換成列）；以及**不可能執行的列**——資料夾無法寫入，後端刻意不替它報價，所以那一格不畫數字（畫了只會是別的模型的價）。兩者都**不准補成 `$0.00`**：0 是讀數，不是「不知道」。換了模型而那個模型沒有報到某一列時，同樣是「沒有金額」，**不得退回預設模型的價**；確認框因此只提供「每一個選得到的列都有報價」的模型。
...
**⚖️ 2026-09-14 裁定（Alexyu，dsr-9）：陰影只屬於浮層，一個例外都沒有。** 下面三階全部是浮層用的；沒有「坐在頁面上」的東西可以拿陰影。

- **`--shadow-lg`**：貼著內容浮起來的那一層——下拉選單、建議清單、懸浮進度卡、hover 時才出現的浮動按鈕。
- **`--shadow-xl`**：**蓋住內容的覆蓋層**——Dialog、Sheet、側板、Modal、詳情頁 hero。這個上限是刻意的，**沒有比 xl 更重的一階**。
- **`--shadow-sm` / `--shadow-md`**：⚠️ **目前沒有指定用途。** 它們原本分別指向「按鈕」與「`Card` 基本元件」，兩個用途都在 2026-09 被移除（見下方裁定紀錄）。token 保留在 `styles.css` 不刪，因為日巡那組是兩層墨的階梯、拆掉會破壞 lg/xl 的推導；但**不要因為它存在就去用它**。

⚖️ **2026-09-14 三個裁定（Alexyu，dsr-9）：**

1. **按鈕不再有陰影。** 原規則給 Primary／Secondary `--shadow-sm`。實測夜行下它是 `0 1px 2px rgba(0,0,0,.3)`——1px 位移、30% 黑，在深墨綠底上幾乎不可觀測。一個看不見的效果不值得讓「陰影＝浮層」這條規則帶一個例外。
2. **海報磚不再有陰影。** `PosterCardV2` 的磚原本帶 `--shadow-md`。它是**全 app 重複次數最多的元素**（每一格、每一頁）；同一個檔案的註解早就寫著「把整格海報抬起來等於把陰影預算花在重複最多的元素上」，但程式碼仍有一道底陰影。重複最多的元素最不該是破例的那個。
3. **裸值（`shadow-lg` 這種 Tailwind 內建值）全面改用 token。** 理由不是風格：日巡的陰影在 2026 年被重做過（改墨色、兩層、總墨量 ≤15.4%），而那次重做**只改到 `--shadow-*` token**。裸值是框架寫死的黑色、不隨主題變，所以**日巡下那 42 處全是黑色瘀青**——正是上一段明文要避免的東西。改完之後由 ESLint 規則 `local/no-raw-shadow` 擋住新的裸值。

普查數字（2026-09-14，排除註解與 token 內文後實測）：改動前裸值 **46 處／35 檔**、token **10 處／7 檔**，比例 4.6:1。46 處裡約 **42 處位置本來就合法**（都是浮層），只是寫法錯；真正放錯位置的只有 4 處。**問題不是紀律，是同一把尺有兩種刻度。**
...
- **Primary：** 泥金填色、`--text-on-accent` 深墨字、**無陰影**（⚖️ 2026-09-14 裁定，見 §Shadow Vocabulary）。hover 到 `--accent-hover`、按下到 `--accent-pressed`。**注意這兩個狀態在日巡是往暗走**。
- **Secondary：** `--bg-tertiary` 填色、主要文字色、**無陰影**（同上）。
...
- **陰影：** 無。陰影保留給真的浮在頁面之上的東西（Dialog／Sheet／Popover／Toast、側板、下拉、懸浮卡），見 §Elevation。⚖️ 2026-09-14 起這條白名單**沒有例外**——按鈕與海報磚都已從陰影名單移除。
...
| 04 · 列表與進度（11）     | ActivityRow-v2、RequestRow-v2、DownloadCard-v2、GenerationProgress-v2、GlossaryRow-v2、GenQueueRow-v2、**ProgressBar**、**TableRow**、**Toast**、**Skeleton**、LibraryCard（dsr-5）                                                                                                                                           |
...
> ⚖️ **2026-09-20 延伸（Alexyu 裁定，dsr-6e）**：同一條線也適用於「產生字幕」同意流程裡的四種分類標記——候選列的路線徽章（抽取／語音辨識）、群組標題的路線徽章、篩選 chip 的費用標記（僅翻譯費／付費）、確認框的模型品質徽章（品質 A／B）。它們說的是「這一列走哪條路」「這個模型量出來幾分」，是屬性不是狀態，一律中性（`--bg-tertiary` 或 `--bg-secondary` 底、`--text-secondary` 字）。這個畫面上剩下的顏色只有：泥金（要按的按鈕、選中的 chip）、赭（超過上限——你要的比會跑的多）、硃砂（資料夾無法寫入、無法估價、開始失敗）。前三種由 `dsr-6e-1` 落地，品質徽章由 `dsr-6e-2`。
