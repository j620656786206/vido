...
Migrated from source command `agent-vibes-background-music` into a Codex skill. Invoke it as `$source-command-agent-vibes-background-music` and manually rewrite any slash-command behavior that depended on provider-specific runtime expansion.
