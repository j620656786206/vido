...
Migrated from source command `agent-vibes-agent-vibes` into a Codex skill. Invoke it as `$source-command-agent-vibes-agent-vibes` and manually rewrite any slash-command behavior that depended on provider-specific runtime expansion.
