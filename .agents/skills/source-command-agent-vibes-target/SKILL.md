...
Migrated from source command `agent-vibes-target` into a Codex skill. Invoke it as `$source-command-agent-vibes-target` and manually rewrite any slash-command behavior that depended on provider-specific runtime expansion.
