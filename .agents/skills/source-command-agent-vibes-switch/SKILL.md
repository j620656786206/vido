...
Use this skill when the user asks to run the migrated source command `agent-vibes-switch`.
...
Migrated from source command `agent-vibes-switch` into a Codex skill. Invoke it as `$source-command-agent-vibes-switch` and manually rewrite any slash-command behavior that depended on provider-specific runtime expansion.
