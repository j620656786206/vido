...
Use this skill when the user asks to run the migrated source command `agent-vibes-agent`.
...
Migrated from source command `agent-vibes-agent` into a Codex skill. Invoke it as `$source-command-agent-vibes-agent` and manually rewrite any slash-command behavior that depended on provider-specific runtime expansion.
