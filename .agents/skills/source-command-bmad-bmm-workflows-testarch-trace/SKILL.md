...
Use this skill when the user asks to run the migrated source command `bmad-bmm-workflows-testarch-trace`.
...
Migrated from source command `bmad-bmm-workflows-testarch-trace` into a Codex skill. Invoke it as `$source-command-bmad-bmm-workflows-testarch-trace` and manually rewrite any slash-command behavior that depended on provider-specific runtime expansion.
