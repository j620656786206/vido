...
Migrated from source command `agent-vibes-effects` into a Codex skill. Invoke it as `$source-command-agent-vibes-effects` and manually rewrite any slash-command behavior that depended on provider-specific runtime expansion.
