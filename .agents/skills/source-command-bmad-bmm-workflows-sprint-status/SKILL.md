...
Use this skill when the user asks to run the migrated source command `bmad-bmm-workflows-sprint-status`.
...
Migrated from source command `bmad-bmm-workflows-sprint-status` into a Codex skill. Invoke it as `$source-command-bmad-bmm-workflows-sprint-status` and manually rewrite any slash-command behavior that depended on provider-specific runtime expansion.
