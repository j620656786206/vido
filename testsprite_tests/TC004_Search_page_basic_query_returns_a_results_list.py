...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /search (http://192.168.50.52:8088/search) and wait for the SPA to load.
        await page.goto("http://192.168.50.52:8088/search", wait_until="commit", timeout=10000)
        
        # -> Enter a known movie title ('Inception') into the visible search input and submit the search (press Enter).
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div/div/div/main/div/div/div/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('Inception')
