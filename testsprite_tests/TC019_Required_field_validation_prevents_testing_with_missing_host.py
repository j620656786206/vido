...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to http://192.168.50.52:8088/settings/qbittorrent to reach the qBittorrent settings form.
        await page.goto("http://192.168.50.52:8088/settings/qbittorrent", wait_until="commit", timeout=10000)
        
        # -> Clear the Host field (index 258) so it is empty, blur the field to trigger client validation, then extract page content to look for a required-field validation message near the host field.
