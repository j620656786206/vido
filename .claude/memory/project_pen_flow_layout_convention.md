...
**spec 畫面（flow-J）不吃 2600 的區塊間距。** j1-d/j2-d 這類設計決策規格單是**變動高度**的長條（j1=2241、j2 因 2026-08-04 的圖示語法區塊長到 2907），內容一長就會撞到下方區塊。加內容後**必須**檢查與下一個區塊的淨空，且**同一列的 spec 畫面要一起移動以維持頂端對齊** —— 單獨移動一張會讓整列變成參差的階梯。移動 frame 的 y **不影響匯出的 PNG**（`get_screenshot` 抓的是節點不是畫布區域），所以重新對齊是零風險、零截圖 diff 的整理動作。
