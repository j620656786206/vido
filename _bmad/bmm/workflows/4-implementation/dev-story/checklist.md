...
**Now enforced as a MANDATORY gate in `dev-story/instructions.xml` Step 7 (Epic 11 Retro AI-1).** Result recorded as `🎭 A11y Pre-Flight:` in Completion Notes. Do not re-add passive checkboxes here — single source of truth lives in the workflow action (backed by `eslint-plugin-jsx-a11y` at warn).
