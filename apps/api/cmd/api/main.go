...
func main() {
	...
	// sub-7-3 + backlog-glossary-seed-existing-library-and-parse-queue: a
	// show's glossary is seeded from its TMDb cast (en-US ↔ zh-TW, names as
	// `source=metadata`) the FIRST time its shared scope is resolved — which
	// happens before the first translation of any title, whether it was
	// already in the library, arrived through the download parse queue, or
	// was just matched by a scan (enrichment resolves once right after the
	// match). Same OpenCC converter as the subtitle safety net, so both agree
	// on phrase choices. Enrichment additionally stores the cast on the row.
