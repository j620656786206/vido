...
//
// Lives in `package services_test` (external test package) so it can
// import both services (the package under test) and subtitle without
// creating a compile-time cycle. External test packages are a separate
// compilation unit, so services_test → subtitle → services is legal
// (services itself still does not depend on subtitle).
//
// Fixtures are a slice rather than a map so failure output is
// deterministic — easier to diff when a regression lands.
...
func TestParseSRT_ParityWithSubtitle(t *testing.T) {
	...
	for _, fx := range fixtures {
		t.Run(fx.name, func(t *testing.T) {
			subBlocks, errSub := subtitle.ParseSRT(fx.input)
			svcBlocks, errSvc := services.ParseSRTToTranslationBlocks(fx.input)
