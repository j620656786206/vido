...
// UpdateSubtitleGenerationStatus writes ONLY status/path/language for a movie
// whose subtitle was GENERATED by the M1 extract-and-translate pipeline, never
// searched (story sub-1-6, closing `backlog-subtitle-status-writer-search-columns`).
//
// Why a second method instead of a flag on UpdateSubtitleStatus: the search
// path genuinely owns `subtitle_search_score` + `subtitle_last_searched` and
// the library UI reads them to show when a title was last looked for. A
// generated sidecar was never scored against provider results and never
// searched for, so stamping those columns would make it indistinguishable from
// a provider hit (sub-1-5b AC #6.3). The shape mirrors
// EpisodeRepository.UpdateEpisodeSubtitleStatus, which was already correct.
func (r *MovieRepository) UpdateSubtitleGenerationStatus(ctx context.Context, id string, status models.SubtitleStatus, path, language string) error {
	return updateSubtitleGenerationStatus(ctx, r.db, "movies", id, status, path, language)
}
