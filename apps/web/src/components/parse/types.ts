...
export const STATUS_CONFIG: Record<ParseStatus, { label: string; color: string; icon: string }> = {
  ...
  parsing: { label: '解析中', color: 'text-[var(--accent-text)]', icon: 'loader-2' },
  success: { label: '已完成', color: 'text-[var(--success-text)]', icon: 'check-circle' },
  needs_ai: { label: '需要處理', color: 'text-[var(--warning-text)]', icon: 'alert-triangle' },
  failed: { label: '失敗', color: 'text-[var(--error-text)]', icon: 'x-circle' },
};
...
export const STEP_STATUS_CONFIG: Record<StepStatus, { label: string; color: string }> = {
  ...
  in_progress: { label: '進行中', color: 'text-[var(--accent-text)]' },
  success: { label: '成功', color: 'text-[var(--success-text)]' },
  failed: { label: '失敗', color: 'text-[var(--error-text)]' },
