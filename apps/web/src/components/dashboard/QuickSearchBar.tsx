  ...
  return (
    <div className={cn('relative', className)} data-testid="quick-search-bar">
      ...
      {showDropdown && recentSearches.length > 0 && (
        <div
          ref={dropdownRef}
          className="absolute left-0 right-0 top-full z-10 mt-1 overflow-hidden rounded-lg border border-slate-700 bg-slate-800 shadow-lg"
          data-testid="recent-searches-dropdown"
          role="listbox"
        >
          <div className="border-b border-slate-700 px-3 py-2">
            <span className="text-xs font-medium text-slate-400">最近搜尋</span>
          </div>
          <ul>
            {recentSearches.map((search, index) => (
              <li key={search}>
                <button
                  ...
                  className={cn(
                    'flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-slate-200 transition-colors',
                    focusedIndex === index ? 'bg-slate-700' : 'hover:bg-slate-700/50'
                  )}
                  role="option"
                ...
                >
                  ...
                  <button
                    ...
                    className="shrink-0 rounded p-0.5 text-slate-500 transition-colors hover:bg-slate-600 hover:text-slate-300"
