...
1. **Seed data** (gating prerequisite — same blocker as the 2026-03 first run): the deployed NAS at `http://192.168.50.52:8088` must contain fixtures for the new cases — mixed-status downloads incl. a **parse-failed** one (TC080/083), an **owned matched movie** (TC084/085), an **unmatched/failed-metadata** item (TC086), and a way to put health into **degraded** (TC088). Extend `scripts/seed-test-data.sh`.
