...
Every Phase-3 FR maps to exactly one epic (16/16, no gaps):
...
PH3-F6 desktop library filter rail      → Epic 0  ux3-foundation    (D2; rail ≠ Discover power-filter PH3-M2)
...
cascade renders against. Reopened for the desktop library filter rail (D2 — replaces the
mobile bottom-sheet misused on desktop).
**FRs covered:** PH3-F1, PH3-F2, PH3-F3, PH3-F6
...
URLs. Delivers the N1 data layer the cascade renders against. Reopened 2026-06-17 for the
desktop library filter rail (D2). **FRs:** PH3-F1/F2/F3/F6.
...
### Story 0.6: Desktop library filter rail design (`.pen` flow-i)

As the design system,
I want the desktop library filter redrawn as a left rail in v2,
So that dev builds the rail against a spec instead of the mobile bottom-sheet misused on
desktop (D2 / PH3-F6).

**Status:** done — PR #89 (squash `68eb939`). Canonical story: `ux3-0-6-library-filter-rail-design.md`.

**What landed** (`ux-design.pen`, flow-i): `I5-D` rail persistent · `I6-D` collapsed→`篩選(n)`
+6-col · `I7-D` states (genre loading skeleton / load-failed fail-soft / no-results).
6 decisions: instant-apply + URL-sync (no apply/reset; mobile sheet keeps batch) · sort in
toolbar · 264px collapsible · active chips above grid · per-section fail-soft · grid
lg3/xl4/2xl5 open vs lg4/xl6 collapsed. Reuses `FilterPanel` fields (類型/類別/年份/未匹配) +
`SortSelector` ONLY (dropped i1-d's invented 地區/最低評分). Token-only, Noto Sans TC +
JetBrains Mono; rail = `$bg-primary` 2nd-level sidebar distinct from `$bg-secondary` nav.
**Boundary:** Library's own-collection filter — NOT the Discover power-filter (PH3-M2 /
ux3-discover-v2).

### Story 0.7: Desktop library filter rail (frontend)
...
**Status:** ready-for-dev. Canonical story: `ux3-0-7-library-filter-rail-frontend.md`. Design
ref: Story 0.6 / #89.

**Acceptance Criteria** (summary — full ACs in the story doc):

**Given** `lg`+ (≥1024px),
**Then** a 264px left rail renders `FilterPanel`'s fields (類型/類別/年份/未匹配) as a 2nd-level
sidebar; **`<lg`** keeps `LibraryFilterSheetV2` unchanged (batch 套用/重置).

**Given** any rail control change,
**Then** it applies instantly + syncs the URL (no apply/reset on desktop; only 清除全部),
numeric params coerced per Rule 26.

**Given** sort,
**Then** `SortSelector` lives in the content-area toolbar (not the rail).

**Given** collapse,
**Then** the rail hides → toolbar `篩選 (n)`; grid reflows lg3/xl4/2xl5 → lg4/xl6.

**Given** active filters,
**Then** a removable `FilterChip` row sits above the grid + 清除全部.

**Given** the genre async load,
**Then** loading skeleton / load-failed inline retry / filtered no-results CTA (each
fail-soft); touch ≥44px, token-only, reduced-motion.

**Sequencing:** after PR #87 (shell) merges; shell-gated; Rule-24 ③ resolution of
v2-followups gap #1.
