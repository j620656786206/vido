...
> **🔶 2026-06-16 重大修訂（見 `../architecture/adr-subtitle-route-c-generation.md` + `../subtitle-v4-replan-and-feasibility-audit-2026-06.md`）：**
> Live POC 證實**「抓取現成繁中字幕」(P1-010/016/017/019) 對繁中無可用來源**：Zimuku WAF 牆死、**Assrt token 實測無法取得（已驗證）**、OpenSubtitles 繁中薄。**下表的「多來源抓取」路線（Route A）對繁中已封死、不再是護城河。** 護城河改為下節 3.3 的 **Route C 生成（轉錄→LLM 翻譯）＋ per-show 術語表 ＋ metadata 在地化**（POC 已實測通過，產出贏過現成字幕）。P1-012（簡繁識別）/P1-013（副檔名）/P1-014（OpenCC）/落檔等引擎元件由 Route C 復用；P1-010/016/017/019 標為 dormant、移除 Zimuku。**新增需求 P1-022 — metadata 在地化（Section E）**：把 .nfo 劇情/分集/角色翻成繁中、以只增不覆蓋方式回寫供 Kodi/Jellyfin/Plex 刮取（復用同一套 LLM＋術語表基建）。
...
#### 3.3 AI 字幕生成（Route C — 繁中核心；2026-06-16 由「可選」升為核心路徑）
