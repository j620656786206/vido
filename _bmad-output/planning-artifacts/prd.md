...
- **Official C++ OpenCC helper `s2twp` runs as a DETERMINISTIC final pass** on the output, guaranteeing Traditional script even if the LLM leaks Simplified characters (idempotent on already-Traditional text; `converter.go` keeps the stable contract). The production path is helper-only; no Go OpenCC binding is shipped. **LLM = semantics, OpenCC = orthography.**
