...
| 2 | **媒體庫 Library ▸ 電影/影集** | ✅ **pilot done (ux2-2/2-3) — do NOT redo** | A browse + B detail | Cleanup only: clean-route `/library/{movies,tv}` split (Foundation #3); pinnable saved views (D2 general mechanism); recs tile → `PosterCardV2`; **Epic 12 detail-block design backfill** (§3). |
