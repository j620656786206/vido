...
- **Stamps [@contract-v1→v2]:** AC #2 — BUMPED by 13-2a (2026-08-19, the bump this line always predicted): `AddSeries` is now selection-aware (`AddOptions.Seasons/Episodes`; empty selection = the v1 whole-series flow byte-identical; partial = selected-only seasons monitored + `monitor:"none"` + explicit episode monitoring + SeasonSearch/EpisodeSearch commands). Queue mapping (`seriesId→ExternalID`) is UNCHANGED. Consumers at bump time: 13-3a (done — FROZEN, consumed only the unchanged queue-mapping half); 13-7a/13-7b (not-done — stale-marked per Rule 20 🔁, see their Dev Notes).
...
| Date       | Change                                                                                                                                                     |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 2026-08-19 | [@contract-v1→v2] AC #2: what changed — `AddSeries` gains selection-aware semantics (13-2a: `AddOptions.Seasons/Episodes` additive fields; partial adds monitor only the selected seasons/episodes and search via explicit SeasonSearch/EpisodeSearch commands; empty selection stays byte-identical to v1); what breaks downstream — nothing at the call-signature level (additive options), but any consumer relying on "AddSeries always monitors the whole series" must re-confirm. Downstream scan: 13-3a done/FROZEN (queue-mapping half untouched); 13-7a/13-7b not-done → stale-marked in their Dev Notes + sprint-status entries (Rule 20 🔁). |
