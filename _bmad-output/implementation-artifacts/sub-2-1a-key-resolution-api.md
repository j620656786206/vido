...
**Given** `Config.HasEncryptionKey()` and that the secrets service is AES-256-GCM over `ENCRYPTION_KEY`, **when** that key is absent, **then** `GET /settings/keys` returns `{..., writable: false, reason: "encryption_key_missing"}` and `PUT` returns **409** with 「未設定加密金鑰，無法安全儲存 API 金鑰」 + suggestion 「請設定 ENCRYPTION_KEY 後重啟」.
...
- ⏱️ **The capability gate is now evaluated per call, not per boot — deliberate, with a measured cost.** `batch.go:316` calls `pipelineAllowed()` once per batch item, so each item now costs one SQLite read + one AES-GCM decrypt instead of a struct-field read. For a 400-episode batch that is roughly 80 ms across a run that takes minutes — negligible, and the semantics are strictly better: a key revoked mid-batch now takes effect on the next item instead of being frozen at the value the process booted with. Recorded so a future reader does not "optimise" it back into a cached boolean and silently restore the staleness this story exists to remove.
...
| Date | Change |
|---|---|
| 2026-08-05 | **Post-merge TestSprite verification + env-var copy fix.** MCP backend run on the seeded :8090 env: TC002 (PUT validation + key-test 409) ✅, TC003 (pipeline gate 409-before-lookup) ✅, TC001 (save→secret→mask→delete round trip) inconclusive on TestSprite infra (codegen `\u` SyntaxError, then tunnel ReadTimeout) but **verified correct by direct HTTP cross-check** — report: `testsprite_tests/testsprite-mcp-test-report.md`. REAL BUG found by the verification: every user-facing/doc reference said `VIDO_ENCRYPTION_KEY`, but the server reads **`ENCRYPTION_KEY`** (`config.go:138`, `docker-compose.yml:69`) — an operator following the 409 suggestion verbatim changed nothing (the silent-no-op class this story removes). Fixed the suggestion string, Swagger comments, code comments, this story's AC #4, the epic AC, and the 2-1b draft; pinned by a `NotContains(VIDO_ENCRYPTION_KEY)` handler assertion. |
