...
## 七、複審結果（Claude，MCP 唯讀，2026-09-10）

10 張畫面 + 10 個 caption 全部存在、座標正確、文案逐字符合本文件的文案表。
`ctx.problems` 只有活動頁背景區塊「部分裁切」，那是 `f15-d-v2` / `f15-m-v2` 本來就有的
（頁面比 900 高的視窗長，本來就要捲），**不是這一輪造成的**。
F21-M 的驗收條件通過：`headline` 在螢幕 y≈170、`開始` 在 y≈788–832，844 高的視野內同時看得到。
J8-D 底部 y=25327，離下一個區塊（y=27381）還有 2,054px，沒有相撞。

複審抓到三個要改的地方，全部是**我提示詞寫得不夠精確**造成的，不是執行的錯：

| # | 畫面 | 問題 | 為什麼要改 |
| --- | --- | --- | --- |
| 1 | F21-D | 「已排除」兩列都掛著「未匹配」小標 | 提示詞叫你複製 `row-星際效應`，那一列原本在 F15 的「未匹配」段，標籤跟著被複製過來。結果稿上讀起來像「已排除＝未匹配」，但排除是使用者的決定，跟 TMDb 有沒有比對到無關。而且手機版 F21-M 沒有這個標籤，同一張稿的兩個版本互相矛盾。 |
| 2 | F23 | 砍線下方的「影集（12 部）」群組沒有淡化 | 這張稿自己的註腳寫「線以上會跑，線以下會等」，可是線以下最大的那一塊是滿亮度的。稿子在同一個畫面裡自相矛盾。 |
| 3 | F23 | 影片列的路線標籤寫「抽取」，F21 寫「僅翻譯費」 | 同一個流程的兩張稿，對同一件事用了兩套詞。群組列已經是「僅翻譯費」，只有影片列還是舊詞。 |

---

## 提示詞 K — 複審修正（三處，全部是 Update／Delete，沒有新設計）

> 貼給 Pencil Inline AI Agent 的內容從這裡開始。

在 `ux-design.pen` 做以下三件事，全部是既有節點的修改，不要新建任何節點。

**1. F21-D-v2：拿掉「已排除」兩列的「未匹配」小標。**
刪除節點 `K2W2Ra`（`row-excluded-星際效應` 裡的 `chip-unmatched`）
與節點 `o78Cq`（`row-excluded-教父` 裡的 `chip-unmatched`）。
兩列其他東西（海報字、片名、時長、`僅翻譯費` 徽章、金額、`復原` 按鈕、`opacity: 0.6`）全部不動。

**2. F23-D-v2：砍線下方的影集群組要淡化。**
把節點 `dnCMH`（`group-影集`）的 `opacity` 設成 `0.6`。
**勾選狀態不要動** —— 它仍然是已同意的，只是會被暫停。

**3. F23-D-v2：影片列的路線標籤改成與 F21 同一套詞。**
把下列五個文字節點的內容從 `抽取` 改成 `僅翻譯費`：
`xDiFm`（沙丘：第二部）、`ZElg1`（奧本海默）、`dCeDc`（全面啟動）、
`NwjB5`（教父）、`MkJbY`（星際效應）。
字級、字重、顏色都不動（11/600、`$success`）。
改完後請確認這五列右側沒有被擠爆 —— 標籤變長了，若右側放不下，
把片名那一欄縮窄即可（片名本來就是 `fill_container`），**不要**縮字級。

> 提示詞 K 結束。

跑完存檔（⌘S）後告訴我，我會重跑一次複審，然後重出截圖、更新 export script、開 PR。

---

## 八、第二輪複審（Claude，2026-09-10）

提示詞 K 的三處修正全部落地並經 MCP 逐項確認：
`row-excluded-*` 的 `chip-unmatched` 已刪（兩列都是 0 個）、`group-影集` 的 `opacity` = 0.6
（線以上的 `group-已匹配電影` 維持 1，正確）、F23 五個影片列的路線標籤都變成 `僅翻譯費`
（11/600、`$success`，與 F21 同一套詞）。`ctx.problems` 只剩活動頁背景那個原本就有的裁切。

截圖比對後發現：10 張裡有 7 張你已經自己重出並 commit 了，而且與我這次重出的**位元完全相同**。
缺的只有 F22-D、F26-D、F27-D 三張（提示詞 A/B/C 那一批），已補上。

看圖時抓到**一個我提示詞寫錯的地方**：

**F27 掃描完成卡的視覺層級是反的。** 我在提示詞 C 裡把「批次生成字幕 →」指定成
`$text-secondary`（灰色），可是同一張卡上原本就有的「查看未比對項目」「查看錯誤」
是 `$accent-primary`（藍色）。結果變成：整張卡最不顯眼的那個連結，正好是
**沒有內嵌字幕的影片唯一能走的那條路**。這正面牴觸裁定 1「次按鈕不得消失」——
把它畫得比「查看錯誤」還淡，等於用顏色把它藏起來。

---

## 提示詞 L — F27 動作列的顏色修正（一個屬性）

> 貼給 Pencil Inline AI Agent 的內容從這裡開始。

在 `ux-design.pen` 的 `f27-d-v2` 畫面裡，找到 `toast-actions` 裡名為 `actionManual`
的文字節點（內容是 `批次生成字幕 →`）。

把它的 `fill` 從 `$text-secondary` 改成 `$accent-primary`，
與同一列的 `actionUnmatched`、`actionErrors` 一致。

`fontWeight` 維持 `normal` 不要動 —— 主次的差別由第一個項目
`actionGenSubs`（`依政策執行 →`，`fontWeight: "600"`）自己扛，
不需要靠把別人變灰來取得。其他三個動作一律不動。

> 提示詞 L 結束。

跑完存檔（⌘S）後告訴我，我會重出 F27 那一張、然後開 PR。

---

## 九、第三輪複審與結案（Claude，2026-09-10）

提示詞 L 落地：`actionManual` 現在是 `$accent-primary`、13/normal，與 `actionUnmatched`、
`actionErrors` 同色；`actionGenSubs` 維持 `$accent-text` 13/**600**，主次由字重承擔。
`f27-d-v2` 無裁切。

**殘留、刻意不追的一點：** `actionManual` 的字重是 `normal`，兩個診斷連結是 `500`。
13px 下這個差異看不出來，而且顏色（真正的問題）已經修好；再開一輪 round-trip
換不到可感知的差別。記在這裡是為了不讓它變成「沒人知道的偏差」。

**截圖歸屬：** 10 張裡有 8 張由 Alexyu 在各自的 `.pen` commit 裡一併重出並提交
（F21-D／F21-M／F22-M／F23／F24／F25／F27／J8）；本次補上 F22-D 與 F26 兩張。
全量重出 184 張，其餘 149 張是 re-render 雜訊，已全部 `git checkout` 還原。

**`SCREENS` 歸屬：** Alexyu 已自行加入 7 筆；本次只補缺的 3 筆
（`S25U3A`／`ePHJo`／`U03low`）。加完 184 個 key、零重複，與 184 張截圖數目一致。
