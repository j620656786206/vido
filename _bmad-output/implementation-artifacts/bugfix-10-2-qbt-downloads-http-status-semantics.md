...
| 2026-05-07 | CR Amelia `/code-review` COMPLETE → status `review` → `done`. 8 findings auto-fixed: **H1** added `ErrCodeAuthFailed` arm to `GetDownloadDetails` (BE handler symmetry — UX message regression closed; new test assertions on `response.Error.Message`/`Suggestion`). **M1** added `tests/e2e/downloads.spec.ts` + `automation-summary-bugfix-10-2.md` to File List below. **M2** AC #4 `swag init` waiver explicitly accepted by user (no Swagger setup in `apps/api/` per project-context.md Step 1.2 still pending — annotations updated in source serve as accurate contract docs until Phase 1.2 lands; AC #4 substance accepted, deliverable deferred). **M3** `qbtConfigResult` typed as `Partial<ReturnType<typeof useQBittorrentConfig>>` so future hook signature changes raise TS errors instead of silent runtime mismatches. **L1** `slog.Warn("unknown qBT error code mapped to 502", ...)` in `qbtErrorToHTTPStatus` default branch for observability of future qBT codes. **L2** extracted `SetupRequiredMarker = "SETUP_REQUIRED"` const in handlers package; replaced literals at handler (×3) and test (×3) sites. **L3** documented fail-closed gate behavior on `useDownloads` JSDoc — unconfigured/loading/errored qBT config silently suppresses fetch (intentional, surface lives in `useQBittorrentConfig` consumers). **L4** replaced 7× `setTimeout(r, 200)` absence-asserts with `await waitFor(() => fetchStatus === 'idle')` — spec runtime dropped 1.4s → 402ms (3.5× speedup). Regression gates: `go test ./internal/handlers/` PASS (all matrix + new H1 assertions green), `vitest run useDownloads.spec.ts` 19/19 PASS. 🔒 Rule 7 Wire Format: PASS (0 codes added/modified — extracted const wraps existing wire literal). |
...
- `apps/api/internal/handlers/download_handler.go` — added `qbtErrorToHTTPStatus` helper (8 lines) + `net/http` import; replaced 3 switch arms (~lines 80, 175, 209); appended `SETUP_REQUIRED` to 2 NotConfigured suggestion strings (`ListDownloads` + `GetDownloadCounts` + `GetDownloadDetails`); updated 3 endpoint Swagger `@Failure` annotation blocks (removed `400`, added `502/503/504`). **CR fixes (2026-05-07):** added `SetupRequiredMarker` const + replaced 3 literal sites; added `slog.Warn` to `qbtErrorToHTTPStatus` default branch (L1 observability); added explicit `ErrCodeAuthFailed` arm to `GetDownloadDetails` (H1 — restores UX symmetry across the 3 GET endpoints).
- `apps/api/internal/handlers/download_handler_test.go` — flipped 9 `StatusBadRequest` assertions to the new mapped statuses (`StatusServiceUnavailable` / `StatusBadGateway` / `StatusGatewayTimeout`); added 2 `SETUP_REQUIRED` substring assertions for NotConfigured cases; appended `qbtStatusMatrix` table + 3 table-driven tests (`TestDownloadHandler_{ListDownloads,GetDownloadDetails,GetDownloadCounts}_QBTErrorStatusMatrix`, 4 codes × 3 endpoints = 12 assertions); added `TestDownloadHandler_GetDownloadDetails_TorrentNotFound_Unchanged` regression guard for the inline 404 branch. **CR fixes (2026-05-07):** replaced 3 literal `"SETUP_REQUIRED"` with the `SetupRequiredMarker` const; strengthened `TestDownloadHandler_GetDownloadDetails_AuthFailure` with H1 regression guard asserting on `response.Error.Code` + `Message` (`"qBittorrent 認證失敗"`) + `Suggestion` (`"請檢查帳號密碼是否正確。"`).
- `apps/web/src/hooks/useDownloads.ts` — imported `useQBittorrentConfig`; gated `useDownloads` (`enabled: isConfigured`, `refetchInterval` masked by `isConfigured`); gated `useDownloadCounts` (effective `enabled = enabled && isConfigured`); gated `useDownloadDetails` (`enabled: !!hash && isConfigured`); single-line `bugfix-10-2:` comment on each `enabled:` clause. **CR fix (2026-05-07):** added JSDoc block on `useDownloads` documenting the fail-closed gate behavior (L3 — explicit signal that unconfigured/loading/errored qBT config silently suppresses fetch by design).
- `apps/web/src/hooks/useDownloads.spec.ts` — added `vi.mock('./useQBittorrent', ...)` at top, mocked `useQBittorrentConfig` with default `{ data: { configured: true } }` via per-describe `beforeEach`; added 7 new gate-coverage tests under the new `describe('useDownloads - qBT config gate (bugfix-10-2)', ...)` block; updated one error-handling test message from `400` → `502` to reflect the contract. **CR fixes (2026-05-07):** retyped `qbtConfigResult` helper as `Partial<ReturnType<typeof useQBittorrentConfig>>` (M3 — drops `as unknown as` double-cast, future hook signature changes raise TS errors); replaced 7× 200ms `setTimeout` absence-asserts with `waitFor(() => fetchStatus === 'idle')` (L4 — spec runtime 1.4s → 402ms).
- `tests/e2e/downloads.spec.ts` — **(CR M1, was missing from File List)** added `beforeEach` mock of `/api/v1/settings/qbittorrent` returning `configured: true` so existing tests still fire downloads after the bugfix-10-2 hook gate landed; added new describe `Downloads qBittorrent config gate (bugfix-10-2)` with 2 P1 E2E tests proving (a) zero `/api/v1/downloads*` requests when `configured: false` and (b) requests resume after gate flips open. Browser-level proof of AC#7.
- `_bmad-output/implementation-artifacts/automation-summary-bugfix-10-2.md` — **(CR M1, was missing from File List)** automation-summary auxiliary doc generated alongside the BE/FE work.
- `_bmad-output/implementation-artifacts/sprint-status.yaml` — bugfix-10-2 status `ready-for-dev` → `in-progress` → `review` → `done`.
