...
- `apps/web/src/components/homepage/ExploreBlock.tsx` — accepts `ownership: OwnedMediaState` prop from parent (CR H1 fix — hoisted), passes `isOwned(item.id)` / `isRequested(item.id)` to each `PosterCard`
- `apps/web/src/components/homepage/ExploreBlock.spec.tsx` — updated to supply the new `ownership` stub in all render helpers
- `apps/web/src/components/homepage/ExploreBlocksList.tsx` — CR H1 fix: `useOwnedMedia` hoisted here via `useQueries` over shared `exploreBlockKeys.content(id)` — single POST unions TMDb IDs across all blocks (AC #4 literal fulfilment)
- `apps/web/src/components/homepage/ExploreBlocksList.spec.tsx` — new regression guard `hoists availability lookup — one useOwnedMedia call unions TMDb IDs across blocks`
...
| 2026-04-17 | CR Amelia `/code-review` (adversarial). 2 HIGH + 4 MED + 2 LOW findings; 7 fixed, 1 (M3 shared fetchApi) retracted after discovering the `fetchApi` pattern is duplicated across all 19 services repo-wide (repo-level tech debt, not a Story 10-4 issue). **H1** AC #4 literal fulfilment — `useOwnedMedia` hoisted from each `ExploreBlock` into `ExploreBlocksList` via `useQueries` over shared content keys. With N blocks, a multi-block homepage now issues a single `POST /media/check-owned` whose body unions every visible TMDb ID. **H2** Handler error-message `"max 500"` swapped to `fmt.Sprintf(..., availabilityCheckOwnedMaxIDs)` so the constant is the single source of truth. **M1** `useOwnedMedia` memoises the `owned` Set, both predicates, and the return object so `React.memo`'d consumers stay memoised across renders. **M2** Misleading "defensive dedupe" comment in `AvailabilityService` rewritten to reflect that TMDb's id-space is type-scoped, so the same integer can legitimately identify both a movie and a TV show — dedupe is load-bearing, not defensive. **M4** Added `ctx.Err()` short-circuit between the movie and series repo calls in `AvailabilityService` so abandoned client connections don't keep burning DB work. **L1** `AvailabilityBadge` now carries `role="status"` + `aria-live="polite"` so the badge's async appearance is announced to screen-reader users. **L2** Error-path test bumped 3s → 5s timeout and documented the retry-override at wrapper level to prevent CI flake. New regression guard test: `hoists availability lookup — one useOwnedMedia call unions TMDb IDs across blocks`. Full gate PASS: nx test api + nx test web 1722/1722 (+1 from H1 guard) + lint:all 0 errors + prettier clean. Story status `review` → `done`. |
