# Spike — 字幕管線順序裁定證據 (2026-08-06)

**目的:** 為 M2「把管線串成三層 fallback」裁定 ②③ 的順序。
**執行:** Amelia (dev),Alexyu 指示,實測於 Unraid NAS + 本機。
**狀態:** ⛔ 原結論已由 §0-A 修正撤回,待重新量測後再裁定。

---

## ⛔ 0-A. 重大修正(2026-08-06 稍晚)—— 以下三項推翻本文原結論

本文初版的樂觀結論建立在**三個有缺陷的量測**上。修正後 B 的價值大幅縮小,
**順序建議暫時撤回,待重新量測**。原文保留於後,作為修正的對照。

### 修正 1:85% 命中率不適用於實際落到搜尋層的檔案

85% 是拿**刻意篩選出的純外語樣本**量的(§5)。當時的理由是「華語內容不需要翻譯,
拿它們拉低命中率是誤導」——**該推論錯誤**。華語內容不需要「翻譯」,但**確實需要字幕**,
而且經重新調查,它們正是落到 ②③ 層的主力:

| 落到 ②③ 的作品組成(39 部不同作品) | 佔比 | Assrt 命中率 |
|---|---:|---:|
| 華語原生(國漫 / 陸劇) | **30 部 · 77%** | **6%** |
| 外語作品 | 9 部 · 23% | 85% |

**期望命中率 = 0.77 × 6% + 0.23 × 85% ≈ 24%**,不是 85%。

### 修正 2:60 秒閘門有跨集數偽陽性

§6 宣稱「零偽陽性」,但那只驗過**跨電影**負控(校正量 331–785s)。
**跨集數**負控實測:搜 `The Boys S05E06`,`The.Boys.S05E01.ass` 以 **+58.76s** 通過 60 秒閘門。
同劇不同集有相同片頭、相似節奏,alass 對得進去。真正的正控落在 ≤9.1s,
**門檻應收至 15–20s 量級,但尚未以足量跨集負控校準**。

**「零偽陽性」宣稱撤回。**

### 修正 3:內嵌中文軌實測 12/12 為簡體

§3 的「有中文文字軌」僅依 ffprobe 的 `chi`/`zho` 標記,無法區分簡繁。
實際抽出內容判定(比照 `detector.go`,繁體專用字比例):**12 個樣本全部繁比 0.00**。

所以「13% 有中文字幕」實為「13% 有**簡體**中文軌」,層① 的產出仍須 OpenCC s2twp。
連帶產生一個本文未涵蓋的產品問題:**層① 抽到簡體時,是否仍應試層②(人工繁中)?**

### 連帶:端到端測試樣本無效

首輪 12 集端到端測試以「有無 sidecar」挑選,但那與管線分層條件無關。
經 ffprobe 複查,12 集中僅 1 集真正落到 ②③;被當作「端到端成功」的
The Last of Us S02E01 **本身就有 3 條中文內嵌軌**,層① 即可處理,根本不會走到搜尋。

正確的取樣條件為:`無中文文字軌 AND 無外語文字軌 AND 無 sidecar`。
重新調查後該母體為 **142/208 = 68.3%**(片庫掃描的 67.2% 得到確認,錯的只有集數挑選)。

---

## 0. 一句話結論(⚠️ 已被 §0-A 修正,保留作對照)

> **建議改 B(抽 > 搜尋 > ASR)。** 兩個支撐證據皆已實測:
>
> 1. 選 A 的理由「線上搜尋命中率低」不成立 —— 外語內容 Assrt 命中率 **85%**,其中 **75% 直接帶繁體**。
> 2. B 唯一的風險「搜到的字幕可能不同步」**已解決** —— `alass` 可當閘門,正確與錯誤字幕的校正量差兩個數量級,且無偽陽性,執行成本 2 秒。
>
> **前置條件:先修好 Assrt provider(search 解析目前完全壞掉)。**

---

## 1. 被推翻的前提

`vido-subtitle-pipeline-spec.md:21` 的定案:

> 優先序 = 抽內嵌 > ASR > 線上搜尋(Alexyu 定案)。理由:…線上搜尋命中率低,列為最後備援。

`sprint-status.yaml:374` 的墓誌銘:

> Route A fetch confirmed non-viable for 繁中 (Assrt token unobtainable, …)

**兩者都已失效:** token 現在申請得到(2026-08-06 實測可用);命中率經實測為 85%(外語內容)。
當年沒有量到真實命中率,是因為(a)拿不到 token,(b)就算有 token,Vido 的解析程式也是壞的(見 §4)。

---

## 2. ASR 成本(已量化)

Unraid NAS,`scripts/whisper-nas-benchmark.sh`,10 分鐘真實對白音軌:

| 引擎 | wall | xRT | 每 45 分鐘一集 |
|---|---|---|---|
| faster-whisper **base** int8 | 9s / 600s | **66.7×** | **0.7 分鐘** |
| faster-whisper **small** int8 | 27s / 600s | **22.2×** | **2.0 分鐘** |

硬體:`12th Gen Intel Core i5-12400` · 12 threads · 31 GiB · `/dev/dri` 存在。

**⚠️ 不可外推:** 這台遠強於 spec 假設的「主流 NAS」(J4125/N5095,4 核無 AVX2)。
結論只適用於此部署;spec 對低階機的判斷仍然成立,產品化需要算力感知(spec §6)。

**OpenVINO iGPU:未測。** NAS 的 `docker.img` 只剩 3.4G(40G 已用 36G,37 個 image / 38 個容器),
pip install 失敗 `No space left on device`。CPU 已足夠快,不值得為此動使用者的 docker.img。

### benchmark kit 修掉的 4 個 bug

kit 自 2026-07-05 建立以來從未成功執行過,原因是四個彼此遮蔽的問題:

1. `linuxserver/ffmpeg` 跑 s6-overlay,容器內降權到 `nobody`,寫不進 root 擁有的工作目錄
   → `--user 0:0` **無效**,正解是 `chmod 0777 "$WORK"`
2. 新版 Speaches **不再自動下載模型**,需先 `POST /v1/models/{model_id}`(`WHISPER__MODEL` 只預載已在磁碟的模型)
3. `-ss 300` 取樣點在片頭,常是配樂無對白 → 轉錄回空字串,量到的是 VAD 跳過而非轉錄
   → 改為可調參數,預設 `1200`
4. WhisperLive image 7.48GB,`docker run -d` 後只 `sleep 25` 就連線 → 無限等待
   → 需就緒輪詢;另 Unraid 主機**沒有 python3**,腳本原本用 python3 取 text head

修正已 commit 進 `scripts/whisper-nas-benchmark.sh`,含成因註解。

---

## 3. 片庫組成(決定各層的服務範圍)

取樣 125 檔,母體 2494(已排除 `adults/` 目錄 2817 檔 —— 首次取樣未排除,數字失真,已重做):

| 內嵌字幕軌 | 數量 | 佔比 |
|---|---:|---:|
| 有中文文字軌 | 21 | 16.8% |
| 有文字軌但非中文(可翻譯) | 20 | 16.0% |
| 只有圖片型(PGS/VobSub) | 6 | 4.8% |
| **完全無字幕軌** | **78** | **62.4%** |

- **第 ① 層(抽內嵌)服務 32.8%**
- **67.2% 掉到 ②③** —— 順序決定三分之二片庫的命運

片庫幾乎全是影集(取樣 122 tv / 3 movies;電影母體僅 60 檔)。

---

## 4. Assrt provider 目前完全壞掉 🔴

### 4.1 兩個型別宣告錯誤(`providers/assrt.go`)

以真實 API 回應餵入實際結構,驗證於 `scratchpad/assrtprobe`:

```
有結果 → UNMARSHAL FAILED: cannot unmarshal object into
         Go struct field assrtSearchItem.sub.subs.lang of type string
無結果 → UNMARSHAL FAILED: cannot unmarshal object into
         Go struct field assrtSearchSub.sub.subs of type []assrtSearchItem
```

| 欄位 | Vido 宣告 | 實際 |
|---|---|---|
| `lang` (`:87`) | `string` | 物件 `{"langlist":{...},"desc":"简"}` |
| `subs` (`:79`) | `[]assrtSearchItem` | 有結果=陣列,**無結果=`{}`** |

`engine.go:249` 對 provider 錯誤只 `slog.Warn` 後繼續 → **Assrt 永遠貢獻 0 筆,且無人察覺。**

### 4.2 回應有兩套 schema,官方文件只描述其中一套

同一個 `/sub/search` 端點會回傳兩種不同形狀的項目:

| | schema A(文件版) | schema B(未文件化) |
|---|---|---|
| 語言 | `lang.langlist` + `lang.desc` | `m_langn: [...]` + `m_lang: "英&nbsp;简&nbsp;双语"` |
| 識別 | `id` / `native_name` / `videoname` | `fileid` / `sub_name` / `m_version` |

實測 `q=The Boys S05E06` 的 14 筆結果 **全部是 schema B**;`q=The Matrix` 則回 schema A。

### 4.3 現成的相容性對照表

第三方實作 [`dyphire/mpv-sub-assrt`](https://github.com/dyphire/mpv-sub-assrt) 獨立遇到並繞過同一問題,
`sub-assrt.lua:514` 對三個欄位都做 fallback —— 可直接抄:

| 用途 | fallback 鏈 |
|---|---|
| 名稱 | `video_chinese_name` → `native_name` → `videoname` |
| 語言 | `lang.desc` → `m_lang`(需 `gsub("&nbsp;", " ")`) |
| 識別碼 | `id` → `fileid` |

其查詢形式為 `q=<短查詢>&no_muxer=1&pos=N`,**不使用 `is_file`** —— 與本 spike 的實測最佳解一致。

### 4.4 損壞範圍是侷限的

| 元件 | 狀態 |
|---|---|
| `Search` 回應解析 | 🔴 壞 |
| 查詢構造 | 🟡 可用,但缺 `no_muxer=1` |
| `/sub/detail` 解析 | ✅ `URL`/`Filename` 與 `Filelist` 兩形狀都已處理 |
| 下載/解壓 | ✅ |
| 速率限制 | ✅ 13s 間隔正確 |

### 4.5 文件不可信

官方文件寫配額 **20/min**,實測為 **5/min**(`/v1/user/quota` 打一次即從 5 掉到 4)。
`assrt.go:23-27` 的註解(2026-07-31 實測)是對的。**未來勿依文件調速率。**

---

## 5. Assrt 命中率(實測)

方法:繞過壞掉的 provider 直接打 API;短查詢(`Show SxxExx` / `Title Year`),雙 schema 判讀;
以使用者瀏覽器驗證過的 `The Boys S05E06`(14 筆 / 13 中文 / 3 繁)作自檢閘門,不通過則拒絕跑批次。

| 樣本 | n | 有中文 | 含繁體 |
|---|---:|---:|---:|
| **外語作品** | 20 | **17 (85%)** | **15 (75%)** |
| 混合片庫(含華語原生) | 20 | 5 (25%) | 5 (25%) |
| └ 其中華語原生內容 | 16 | 1 (6%) | — |

**分裂的原因:** 射手網是中文字幕站,索引的是**給中文觀眾看的外語作品**。
華語劇/國漫沒有中文字幕組上傳 —— 而它們本來就不需要「外語→中文翻譯」,
拿它們拉低命中率對 ②③ 順序的決策是誤導。**外語作品那一欄才是決策依據。**

3 筆未命中中有 2 筆疑似查詢構造問題(`House of Cards US`、`Overlord S02E03` —— 同劇 S04E01 命中),
真實命中率可能高於 85%。

### 5.1 ⚠️ 搜尋命中 ≠ 拿得到可用字幕

85% 量的是**搜尋命中率**。實際端到端(搜尋 → detail → 下載 → 解壓 → 解析)還有耗損。
對 §6 的 4 部 ground truth 走完整流程:

| 片 | 搜尋 | 下載 | 結果 |
|---|---|---|---|
| F1 (2025) | 6 筆 / 4 繁 | ✅ | **可直接使用**,與 NAS 既有字幕偏移差 **0.01s**,句數 0.98× |
| 哪吒之魔童闹海 (2025) | **0 筆** | — | 華語原生內容,符合 §5 的 6% |
| Superman (2025) | 5 筆 / 3 繁 | ✅ 152KB | ❌ **ASS 格式**,Vido 無 ASS→SRT 轉換路徑 |
| Ballerina (2025) | 3 筆 / 3 繁 | ⚠️ | ❌ 壓縮檔內無 `.srt/.ass/.ssa`(疑為 VobSub 圖片型) |

有結果的 3 部中,**僅 1 部可直接使用**;1 部格式可修;1 部可能本質不可用。
樣本過小不足以定出端到端可用率,但足以證明**耗損存在且不可忽略** —— M2b 的規劃不可直接
把 85% 當作可用率。

**新增的整合缺口:下載路徑缺少格式正規化。**
`engine.go` 的 `convertIfNeeded` 只做 OpenCC(簡→繁),不處理容器格式。
抽取路徑已用 `ffmpeg -c:s srt` 做 ass/ssa→srt(sub-1-4 Finding 2),
**同一套機制應接到下載路徑**。此項應併入 §4 的 Assrt 修復 story。

### ⚠️ 方法論教訓(v1 量測全數作廢)

第一版探針量到 8%,**完全錯誤**,兩個 bug 疊加:

1. 用完整發行檔名 + `is_file=1` 查詢 —— 實測會把 `The Boys S05E06` 從 14 筆砍到 4 筆,多數標題砍到 0
2. 分類器只認 schema A,對 schema B 一律判為「非中文」

使用者以瀏覽器搜尋同一部片得到 14 筆結果,才揭露此錯誤。
**教訓:任何外部 API 的批次量測,必須先在已知案例上通過自檢再開跑。** v2 已內建此閘門。

---

## 6. 同步驗證:**可行**(alass,2026-08-06 實測)

### 6.1 結論

> **`alass` 可以當閘門,判準是「它需要施加的校正量」。**
> 正確字幕需要的校正 ≤ 9.1 秒;錯誤字幕(他片)需要 331–785 秒。**差兩個數量級,完全分離。**
> 執行成本 **1.6–2.4 秒 / 2 小時電影**,可忽略。

### 6.2 方法

素材:NAS 上 4 組 ground truth(電影 + 其自帶 sidecar `.srt`,母體 25 組)。
在 NAS 抽成 16kHz mono 32kbps mp3(4 部共 128MB)傳回本機,alass 在本機跑 —— 不佔 NAS CPU。
全部使用 `--no-split`(見 §6.4)。

每組跑四種情境:

| 情境 | 輸入 | 預期 |
|---|---|---|
| 正控 | 它自己的字幕,未動 | ≈ 0 |
| 偏移控 | 同上 +5 秒 | 正控 − 5.00s |
| **負控** | **另一部片的字幕** | **可辨識的異常** |
| 幀率 | 時間軸 ×(25/23.976) | 非固定偏移,最難的真實案例 |

### 6.3 結果

| 情境 | #1 | #2 | #3 | #4 |
|---|---:|---:|---:|---:|
| 正控 | -4.08s | +0.00s | +4.87s | -0.18s |
| 偏移控 +5s | -9.08s | -5.00s | -0.12s | -5.18s |
| **負控(他片)** | **+387.68s** | **+619.59s** | **-331.66s** | **-785.04s** |
| 幀率 23.976→25 | -4.08s ✓ | -586.15s | -268.78s | -421.59s |

**「偏移控 − 正控」四組皆精確等於 -5.00s** —— alass 的量測精度與重現性都可靠。

**附帶發現:#1 與 #3 的 sidecar 字幕本來就不同步(4–5 秒),alass 會順手修好。**
亦即這個機制不只是閘門,還是既有片庫的字幕品質改善工具。

### 6.4 建議的判準

```
alass --no-split <媒體> <候選字幕> <輸出>
|加權偏移| ≤ 60s  → 採用(且用校正後的版本)
|加權偏移| >  60s  → 拒絕,落到下一層
```

門檻取 60s 是因為觀測到的間隙極大(9.1s → 331s),取中間任一值都安全;需在更大樣本上調校。

`--no-split` 是必要的:允許分段時 #4 正控從 -0.18s 惡化到 -17.14s(切成 5 塊),**分段會傷害正確案例**。

### 6.5 已知限制

**幀率不符是模糊地帶:4 組只救回 1 組**(#1,alass 自行猜對 23.976/25)。
其餘 3 組落在與「他片字幕」重疊的量級,會被門檻誤判為錯誤字幕。

**但誤判方向是安全的**——這是**偽陰性**(丟掉一份其實可用的字幕,落到 ASR),
而**四組負控無一被接受,沒有偽陽性**。最壞情況等同於現行 A 順序的行為,不會讓使用者拿到錯字幕。

改善方向(非阻塞):以 `ffprobe` 取得實際幀率,用 `--sub-fps-inc` / `--sub-fps-ref` 明示後重試一次。

### 6.6 已捨棄的自製方法

| 方法 | 成本 | 結果 |
|---|---|---|
| `ffmpeg silencedetect` | 1595× 實時 | ❌ 素材不適用 |
| 能量包絡互相關 | 1771× 實時 | ⚠️ 6 組偏移只對 3 組,r≈0.07 |

兩者皆在 LibriVox 朗讀素材上測試 —— **連續講話,cue mask 幾乎鋪滿時間軸,沒有對比訊號**。
alass 在同一份素材上**也失敗**(注入 +5s,它回報 -12.27s 並切成兩塊),
證明失敗來自素材而非方法。**教訓:同步驗證只能用真實對白內容評估。**

不建議自行實作 —— alass 已解決此問題,且效能與精度都遠優於手刻。

---

## 7. 裁定建議

### 對外語內容(真正需要翻譯的那部分)

| | 線上搜尋 | ASR + 翻譯 |
|---|---|---|
| 成功率 | **85%** | ~100% |
| 成本 | 一次 API 呼叫 | 2 分鐘 ASR + LLM 翻譯費用 |
| 品質 | **人工字幕** | 機器聽寫,專有名詞易錯 |
| 繁體 | **75% 直接就有**(免 OpenCC) | 需翻譯產生 |
| 風險 | 可能不同步 —— **已由 alass 閘門解決(§6)** | 無 |

**→ B 順序(抽 > 搜尋 > ASR)對外語內容明顯較優。** 85% 的情況下完全免除 AI 成本,且品質更好。
搜尋失敗的成本極低(一次 API 呼叫),落到 ASR 只慢幾秒。

### 對華語原生內容

搜尋命中率僅 6% → 幾乎必然落到 ASR。這是正確的,且 ASR 在此硬體上便宜。
B 順序對這類內容的代價是「多一次註定失敗的 API 呼叫」—— 可接受。

### 但 B 有前置條件

1. **修好 Assrt provider**(§4)—— 這是獨立的一張 story,可先做且不阻塞其他工作
2. ~~解決同步驗證~~ **已解決**(§6)—— `alass --no-split`,|偏移| ≤ 60s 採用,否則落到下一層

### 建議的推進順序

```
1. story: 修復 Assrt search 解析(§4,有現成對照表,範圍侷限)
2. story: M2a —— 接上 ASR fallback 腿(D2 契約 bump:no_text_source 終局→中繼)
3. story: M2b —— 插入搜尋層 + alass 閘門,位置在 ASR 之前(B 順序)
```

同步驗證 spike 已於 2026-08-06 完成,結論為**可行**(§6),故順序建議定為 **B**。
`alass` 應以 vendored 二進位或容器提供;Docker 映像檔需納入。

---

## 8. 連帶待辦

- `sprint-status.yaml:374` 的 Assrt 墓誌銘已過時,應更新
- `9R-S2-nas-whisper-benchmark-spike` 可據 §2 收斂(但需註明硬體不具代表性)
- NAS `docker.img` 使用率 92%,建議擴大或清理(與本 spike 無關的既有風險)
- spec §5「算力感知」的必要性由 §2 的硬體落差進一步佐證
