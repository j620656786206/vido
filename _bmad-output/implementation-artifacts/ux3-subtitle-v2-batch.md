...
- `GET .../status` → `{running, progress|null}`; `POST .../cancel` → `{cancelled, running}`; `GET .../preview?scope=missing` → `{total_items}`.
...
- Per-item stage detail: join existing `transcription_extracting/progress/translation_progress/transcription_complete/transcription_failed` on `current_media_id` (slice-1's `useGenerationProgress` already handles these — frozen stages 提取音訊→轉錄中→翻譯中→簡轉繁→AI校正→完成).
